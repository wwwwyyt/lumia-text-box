...
var dialogue_list_index = 0
var dialogue_list_num = 0
...
func _display_next_dialogue():
	if dialogue_list_index >= dialogue_list_num:
		is_active = false
		script_index += 1
		print("当前剧本播放结束...")
		return
	var dialogue = dialogue_list[dialogue_list_index]
	print("播放对话：", dialogue_list_index)
	
	# 当对话切换成功时，使对话列表索引增加
	# 但是对话切换次数永远比对话数量少一次，因此最后总是需要添加一个空对话
	if _text_transition(dialogue):
...
func add_script(script_index: int=0):
	dialogue_list = scripts[script_index].dialogues
	dialogue_list_num = len(dialogue_list)
